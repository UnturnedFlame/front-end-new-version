const n={WhiteGaussianNoise:"高斯白噪声",polynomial_interpolation:"多项式插值",bicubic_interpolation:"三次样条插值",lagrange_interpolation:"拉格朗日插值",newton_interpolation:"牛顿插值",linear_interpolation:"线性插值",time_domain_features:"单传感器时域特征提取",frequency_domain_features:"单传感器频域特征提取",time_frequency_domain_features:"单传感器时域和频域特征提取",time_domain_features_multiple:"多传感器时域特征提取",frequency_domain_features_multiple:"多传感器频域特征提取",time_frequency_domain_features_multiple:"多传感器时域和频域特征提取",FAHP:"单传感器层次分析模糊综合评估法",FAHP_multiple:"多传感器层次分析模糊综合评估法",BHM:"单传感器层次朴素贝叶斯评估",BHM_multiple:"多传感器层次朴素贝叶斯评估",AHP:"单传感器层次逻辑回归评估法",AHP_multiple:"多传感器层次逻辑回归评估法",feature_imp:"单传感器树模型的特征选择",feature_imp_multiple:"多传感器树模型的特征选择",mutual_information_importance:"单传感器互信息重要性特征选择",mutual_information_importance_multiple:"多传感器互信息重要性特征选择",correlation_coefficient_importance:"单传感器相关系数重要性特征选择",correlation_coefficient_importance_multiple:"多传感器相关系数重要性特征选择",wavelet_trans_denoise:"小波变换降噪",max_min:"max_min标准化",z_score:"z-score标准化",robust_scaler:"鲁棒标准化",max_abs_scaler:"最大绝对值标准化",neighboring_values_interpolation:"邻近值插值",linear_regression:"单传感器线性回归故障预测",linear_regression_multiple:"多传感器线性回归故障预测",deeplearning_interpolation:"深度学习插值",ulcnn_deeplearning:"单传感器一维卷积深度学习模型的故障诊断",random_forest_machine_learning:"单传感器随机森林故障诊断",svc_machine_learning:"单传感器SVM的故障诊断",gru_deeplearning:"单传感器GRU的故障诊断",lstm_deeplearning:"单传感器LSTM的故障诊断",random_forest_multiple_machine_learning:"多传感器随机森林故障诊断",svc_multiple_machine_learning:"多传感器SVM的故障诊断",gru_multiple_deeplearning:"多传感器GRU的故障诊断",lstm_multiple_deeplearning:"多传感器LSTM的故障诊断",ulcnn_multiple_deeplearning:"多传感器一维卷积深度学习模型的故障诊断",spectrumModel_deeplearning:"单传感器基于时频图的深度学习模型的故障诊断",spectrumModel_multiple_deeplearning:"多传感器基于时频图的深度学习模型的故障诊断",dataSource:"数据源",customModule:"自定义模块",private_interpolation:"增值服务组件（插值）",private_fault_diagnosis_deeplearning:"增值服务组件（深度学习故障诊断）",private_fault_diagnosis_machine_learning:"增值服务组件（机器学习故障诊断）",private_fault_prediction:"增值服务组件（故障预测）",extra_feature_selection:"增值服务组件（特征选择）",private_scaler:"增值服务组件（无量纲化）",extra_health_evaluation:"增值服务组件（健康评估）",extra_wavelet_transform:"增值服务组件（小波变换）",private_feature_extraction:"增值服务组件（特征提取）",additional_model_one_multiple_deeplearning:"多传感器信号级加权融合的故障诊断",additional_model_two_multiple_deeplearning:"多传感器信号时频表征自适应加权融合的故障诊断",additional_model_three_multiple_deeplearning:"多传感器特征级融合的深度学习故障诊断",additional_model_four_multiple_deeplearning:"多传感器决策级融合的深度学习故障诊断",additional_model_five_deeplearning:"基于单传感器的知识型 1D 时域深度学习故障诊断",additional_model_six_deeplearning:"基于单传感器的时域和频域协同注意学习故障诊断",additional_model_seven_deeplearning:"基于单传感器的多域深度特征融合故障诊断",nan_detection:"缺失值检测"},e={插值处理:`### 插值处理模块中各个插值算法的比较 
| 插值算法   | 优点                                           | 缺点                                         | 
|--------|----------------------------------------------|--------------------------------------------| 
...
| 层次逻辑回归评估   | 模型的结果易于理解，可以通过系数大小直观地看出不同变量对结果的影响程度。                                                                     | 逻辑回归假设因变量与自变量之间存在线性关系，对于非线性关系可能不适合。由于逻辑回归采用最大似然估计法求解参数，因此对异常值较为敏感。 |`,小波变换:`| 小波变换的小波基类型         | 优点                                                     | 缺点                                              |
|--------------------|--------------------------------------------------------|-------------------------------------------------|
...
`},i={polynomial_interpolation:`# 多项式插值方法
...
### 4. **滤波和去噪**：在信号处理中，多项式插值可以帮助平滑信号，减少噪声的影响。`,neighboring_values_interpolation:`# 邻近值插补在数据预处理中的应用
...
### 5. 缺失数据不多的情况：当数据集中的缺失值不多，且分布均匀时，邻近值插补提供了一种快速且有效的解决方案。`,bicubic_interpolation:`# 双三次插值方法
...
### 3. 能够处理多类分类问题，并且对缺失数据和不完整数据的处理能力较强`,BHM_multiple:`# 层次朴素贝叶斯评估法
## 层次朴素贝叶斯评估法是一种将层次分析和高斯朴素贝叶斯相结合的评价方法，在体系评价、效能评估，系统优化等方面有着广泛的应用，是一种定性与定量相结合的评价模型，一般是先用层析分析法确定因素集，然后用高斯朴素贝叶斯方法建立特征的高斯分布，并最终确定评判效果。
# 算法特点
### 1. 可以综合考虑多个因素的影响，给出全面评价结果
### 2. 对给定的问题能够提供概率形式的决策支持 
### 3. 能够处理多类分类问题，并且对缺失数据和不完整数据的处理能力较强`,FAHP_multiple:`# 层次分析模糊综合评估法
## 层次分析模糊综合评估法是一种将模糊综合评价法和层次分析法相结合的评价方法，在体系评价、效能评估，系统优化等方面有着广泛的应用，是一种定性与定量相结合的评价模型，一般是先用层析分析法确定因素集，然后用模糊综合评判确定评判效果。模糊法是在层次法之上，两者相互融合，对评价有着很好的可靠性。
# 评价过程
### 1. **建立层次结构模型**：首先使用层次分析法确定问题的目标层、准则层和方案层。
### 2. **成对比较和一致性检验**：通过成对比较确定各因素的相对重要性，并进行一致性检验。
...
### 3. **数据驱动**：特征选择过程完全基于数据和模型的反馈，而不是基于领域知识或其他外部标准。`,mutual_information_importance:`# 互信息重要性特征选择
...
### 8. 适应性：特征选择方法应能够适应不同的数据特征和分布情况，保持选择结果的准确性。`,random_forest:`# 随机森林故障诊断
...
### 3. 鲁棒性强:由于采用了卷积和池化操作，该模型对输入数据的微小变化具有一定的鲁棒性。`,wavelet_trans_denoise:`# 小波变换去噪
...
### 快速性：计算过程快速，适合大规模数据集。
### 数据分布敏感：归一化结果依赖于数据中的最小值和最大值，对异常值敏感。`,"z-score":`# z-score标准化
...
### 4. 缩放到[-1, 1]区间：标准化后的数据将位于[-1, 1]区间内，便于比较不同特征的值。`,linear_regression:`# 线性回在趋势预测
...
`,deeplearning_interpolation:`# 基于LSTM网络的深度学习插值算法
...
### 本模块包含算法如下:
 **高斯白噪声： 高斯白噪声(White Gaussian Noise)在通信、信号处理及科学研究等    多个领域中扮演着重要角色。它作为一种理想的噪声模型，具有独特的统计特性和功率谱分布，为系统性能评估、算法测试及信号分析提供了有力工具**`,插值处理:`### 插值处理算法可以对输入信号进行插值操作
***
 ### 插值处理的主要应用有：
 **1. 数值计算。函数逼近：插值方法，如拉格朗日插值多项式，可以用于在给定节点上逼近任意函数，从而在节点外的位置计算函数的近似值。**
     **2. 图像处理。图像放大与缩小：双线性插值等方法可以用于图像的放大或缩小，以获取更高分辨率或适当尺寸的图像。图像平滑：线性插值等方法可以用于平滑图像中的噪声或异常值，提高图像质量。**    **3. 数据净化。缺失值处理：插值法是一种有效的缺失值处理方法，能够根据不同情况选择合适的插值类型进行估算，实现数据的完整性和连续性。数据平滑：插值法可以用于构建连续且平滑的函数模型来拟合数据，消除噪声和异常值的影响，提高数据质量。**
     ### 本插值处理模块中包含算法：
 **多项式插值算法、双三次插值算法、拉格朗日插值算法、牛顿插值算法、线性插值算法**`,TDF:`### 特征提取算法可对输入信号进行人工特征提取
***
 ### 特征提取算法中主要包括信号时域特征和频域特征的提取：
 **1. 时域特征。定义：时域特征描述的是信号随时间变化的关系。在时域中，信号被表示为时间的函数，其动态信号描述信号在不同时刻的取值。    特点：时域表示较为形象与直观，对于正弦信号等简单波形的时域表达，可以通过幅值、频率、相位三个基本特征值唯一确定。时域分析能够直接反映信号随时间的实时变化。**
 **2. 频域特征。定义：频域特征描述的是信号在频率方面的特性。    频域分析是通过对信号进行傅立叶变换等数学方法，将信号从时间域转换到频率域，从而研究信号的频率结构。特点：频域分析能够深入剖析信号的本质，揭示信号中不易被时域分析发现的特征。频域特征通常用于表达信号的周期性信息。**
 ### 本特征提取算法中提取的时域和频域的特征包括：    
 **1. 时域特征：均值、方差、标准差、峰度、偏度、四阶累积量、六阶累积量、最大值、最小值、中位数、峰峰值、整流平均值、均方根、方根幅度、波形因子、峰值因子、脉冲因子、裕度因子**
     **1. 频域特征：重心频率、均方频率、均方根频率、频率方差、频率标准差、谱峭度的均值、谱峭度的标准差、谱峭度的峰度、谱峭度的偏度**`,层次分析模糊综合评估:`### 层次分析模糊综合评估法是一种将模糊综合评价法和层次分析法相结合的评价方法
***
 ### 算法优点：
 **1. 可以综合考虑多个因素的影响，给出全面评价结果。**
     **2. 评价结果是一个矢量，而不是一个点值，包含的信息比较丰富，既可以比较准确的刻画被评价对象，又可以进一步加工，得到参考信息。**
 **3. 模糊评价通过精确的数字手段处理模糊的评价对象，能对蕴藏信息呈现模糊性的资料作出比较科学、合理、贴近实际的量化评价。**`},a={SNR:"信噪比",layers:"网络层数",num_workers:"工作线程数",num_features:"选取特征数量",wavelet:"小波类型",wavelet_level:"小波层数",useLog:"使用训练模型时的标准化方法"};export{a,e as c,n as l,i as p};
